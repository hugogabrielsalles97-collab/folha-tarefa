...
const InputField = ({ label, name, value, onChange, type = 'text', error }: { label: string, name: string, value?: string, onChange: (e: React.ChangeEvent<HTMLInputElement>) => void, type?: string, error?: string }) => (
    <div>
      <label htmlFor={name} className="block text-sm font-medium text-neon-cyan/80 mb-1">{label}</label>
      <input type={type} id={name} name={name} value={value || ''} onChange={onChange} className={`w-full bg-dark-bg border ${error ? 'border-red-500' : 'border-dark-border'} rounded-md shadow-sm p-2 text-white focus:ring-neon-magenta focus:border-neon-magenta`} />
...
const SelectField: React.FC<SelectFieldProps> = ({ label, name, value, onChange, children, error }) => (
     <div>
      <label htmlFor={name} className="block text-sm font-medium text-neon-cyan/80 mb-1">{label}</label>
      <select id={name} name={name} value={value} onChange={onChange} className={`w-full bg-dark-bg border ${error ? 'border-red-500' : 'border-dark-border'} rounded-md shadow-sm p-2 text-white focus:ring-neon-magenta focus:border-neon-magenta`}>
          {children}
      </select>
      {error && <p className="text-red-500 text-xs mt-1">{error}</p>}
     </div>
  ...
  const renderConditionalFields = () => {
    switch (task.discipline) {
      ...
      case Discipline.CONTENCOES:
        return (
          <>
            <InputField label="Frente" name="frente" value={task.frente} onChange={handleChange} />
            <InputField label="Corte" name="corte" value={task.corte} onChange={handleChange} />
          </>
        );
      case Discipline.OAE:
        return (
          <>
            <SelectField label="Obra de Arte" name="obraDeArte" value={task.obraDeArte || ''} onChange={handleChange}>
            ...
            { (task.level === 'Fundações' || task.level === 'Mesoestrutura') &&
              <SelectField label="Apoio" name="apoio" value={task.apoio || ''} onChange={handleChange}>
                <option value="">Selecione o Apoio</option>
                {APOIOS_OPTIONS.map(a => <option key={a} value={a}>{a}</option>)}
              </SelectField>
            }
            { task.level === 'Superestrutura' &&
              <SelectField label="Vão" name="vao" value={task.vao || ''} onChange={handleChange}>
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <SelectField label="Disciplina" name="discipline" value={task.discipline} onChange={handleChange}>
            {Object.values(Discipline).map(d => <option key={d} value={d}>{d}</option>)}
        </SelectField>
        
        <SelectField label="Nível" name="level" value={task.level} onChange={handleChange} error={errors.level}>
      ...
      {taskNameOptions ? (
          <SelectField label="Nome da Tarefa" name="name" value={task.name || ''} onChange={handleChange} error={errors.name}>
              <option value="">Selecione a Tarefa</option>
              {taskNameOptions.map(name => <option key={name} value={name}>{name}</option>)}
          </SelectField>
      ) : (
          <InputField label="Nome da Tarefa" name="name" value={task.name} onChange={handleChange} error={errors.name} />
      )}

      <fieldset className="border border-dark-border p-4 rounded-md">
        <legend className="text-neon-orange px-2">Planejamento</legend>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <InputField label="Início Previsto" name="plannedStartDate" type="date" value={task.plannedStartDate} onChange={handleChange} error={errors.plannedStartDate} />
            <InputField label="Fim Previsto" name="plannedEndDate" type="date" value={task.plannedEndDate} onChange={handleChange} error={errors.plannedEndDate} />
        </div>
      </fieldset>
